...
public class NpgsqlPropertyStore : IPropertyStore, IDisposable
{
    private static readonly XName Property = XName.Get("Property", "https://github.com/ThuCommix/Dav.AspNetCore.Server");
    
    private readonly NpgsqlPropertyStoreOptions options;
    ...
    /// </summary>
    /// <param name="options">The npgsql property store options.</param>
    public NpgsqlPropertyStore(NpgsqlPropertyStoreOptions options)
    {
    ...
    public async ValueTask SaveChangesAsync(CancellationToken cancellationToken = default)
    {
        ...
        foreach (var entry in propertyCache)
        {
            ...
            deleteCommand.Parameters.Add(new NpgsqlParameter("@Uri", entry.Key.Uri.LocalPath));
            ...
            foreach (var propertyData in entry.Value)
            {
                ...
                insertCommand.Parameters.Add(new NpgsqlParameter("@Uri", entry.Key.Uri.LocalPath));
                insertCommand.Parameters.Add(new NpgsqlParameter("@ElementName", propertyData.Key.LocalName));
                insertCommand.Parameters.Add(new NpgsqlParameter("@ElementNamespace", propertyData.Key.NamespaceName));

                if (propertyData.Value.CurrentValue is XElement[] elements)
                {
                    insertCommand.Parameters.Add(new NpgsqlParameter("@ElementValue",
                        new XElement(Property, Array.ConvertAll(elements, input => (object)input)).ToString()));
                }
                else
                {
                    insertCommand.Parameters.Add(new NpgsqlParameter("@ElementValue", propertyData.Value.CurrentValue.ToString()));    
                }
    ...
    public async ValueTask DeletePropertiesAsync(IStoreItem item, CancellationToken cancellationToken = default)
    {
        ...
        await using var deleteCommand = connection.CreateCommand();
        deleteCommand.CommandText = "DELETE FROM dav_aspnetcore_server_property WHERE Uri = @Uri";
        deleteCommand.Parameters.Add(new NpgsqlParameter("@Uri", item.Uri.LocalPath));
...
FROM dav_aspnetcore_server_property WHERE Uri = @SourceUri";
        ...
        copyCommand.Parameters.Add(new NpgsqlParameter("@SourceUri", source.Uri.LocalPath));
        copyCommand.Parameters.Add(new NpgsqlParameter("@DestinationUri", destination.Uri.LocalPath));
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        command.Parameters.Add(new NpgsqlParameter("@Uri", item.Uri.LocalPath));
